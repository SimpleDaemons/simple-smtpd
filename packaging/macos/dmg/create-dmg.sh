#!/bin/bash
# Create DMG installer for simple-smtpd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-smtpd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-smtpd.app"
PKG_NAME="$simple-smtpd.pkg"
...
if [ -f "../assets/icons/$simple-smtpd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-smtpd.icns"
fi
