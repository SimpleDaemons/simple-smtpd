...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-smtpd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-smtpd is a modern, secure implementation of the SMTP protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id smtpddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" smtpddev
    fi
fi
