...
useradd -m -s /bin/bash smtpddev || true
usermod -aG sudo smtpddev || true

# Create project directory
echo "Setting up project directory..."
PROJECT_DIR="/opt/simple-smtpd"
mkdir -p $PROJECT_DIR
chown smtpddev:smtpddev $PROJECT_DIR

# Create log directories
echo "Creating log directories..."
mkdir -p /var/log/simple-smtpd
chown smtpddev:smtpddev /var/log/simple-smtpd

# Create config directory
echo "Creating config directory..."
mkdir -p /etc/simple-smtpd
chown smtpddev:smtpddev /etc/simple-smtpd
...
echo "2. Run: cd $PROJECT_DIR && sudo -u smtpddev ./build.sh"
echo "3. Test with: sudo -u smtpddev make test"
